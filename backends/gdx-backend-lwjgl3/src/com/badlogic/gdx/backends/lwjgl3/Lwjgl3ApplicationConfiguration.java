...
public class Lwjgl3ApplicationConfiguration extends Lwjgl3WindowConfiguration {
	...
	/**
	 * @param visibility whether the window will be visible on creation. (default
	 *                   true)
	 */
	...
	/**
	 * Whether to disable audio or not. If set to true, the returned audio class
	 * instances like {@link Audio} or {@link Music}
	 ...
	 * 
	 * @param simultaneousSources the maximum number of sources that can be played
	 *                            simultaniously (default 16)
	 * @param bufferSize          the audio device buffer size in samples (default
	 *                            512)
	...
	/**
	 * Sets which OpenGL version to use to emulate OpenGL ES. If the given
	 * major/minor version is not supported, the backend falls
	 * back to OpenGL ES 2.0 emulation through OpenGL 2.0. The default parameters
	 * for major and minor should be 3 and 2
	 * respectively to be compatible with Mac OS X. Specifying major version 4 and
	 * minor version 2 will ensure that all OpenGL ES
	...
	/**
	 * Set transparent window hint. Results may vary on different OS and GPUs. Usage
	 * with the ANGLE backend is less consistent.
	 * 
	...
	/**
	 * Sets the polling rate during idle time in non-continuous rendering mode. Must
	 * be positive. Default is 60.
	 */
	...
	/**
	 * Sets the target framerate for the application. The CPU sleeps as needed. Must
	 * be positive. Use 0 to never sleep. Default is
	 * 0.
	 */
	...
	/**
	 * Sets whether to pause the application {@link ApplicationListener#pause()} and
	 * fire
	 * {@link LifecycleListener#pause()}/{@link LifecycleListener#resume()} events
	 * on when window is minimized/restored.
	 **/
	 ...
	 * fire
	 * {@link LifecycleListener#pause()}/{@link LifecycleListener#resume()} events
	 * on when window loses/gains focus.
	 **/
	...
	/**
	 * Sets the directory where {@link Preferences} will be stored, as well as the
	 * file type to be used to store them. Defaults to
	 ...
	 * per-monitor HDPI scale setting. The operating system
	 * may report window width/height and mouse coordinates in a logical coordinate
	 * system at a lower resolution than the actual
	 * physical resolution. This setting allows you to specify whether you want to
	 * work in logical or raw pixel units. See
	 * {@link HdpiMode} for more information. Note that some OpenGL functions like
	 * {@link GL20#glViewport(int, int, int, int)} and
	 * {@link GL20#glScissor(int, int, int, int)} require raw pixel units. Use
	 * {@link HdpiUtils} to help with the conversion if
	 * HdpiMode is set to {@link HdpiMode#Logical}. Defaults to
	 * {@link HdpiMode#Logical}.
	 */
	 ...
	 * GL driver (since GL 4.3), this uses the
	 * KHR_debug, ARB_debug_output or AMD_debug_output extension if available. By
	 * default, debug messages with NOTIFICATION
	 * severity are disabled to avoid log spam.
	 *
	 * You can call with {@link System#err} to output to the "standard" error output
	 * stream.
	 *
	 * Use
	 * {@link Lwjgl3Application#setGLDebugMessageControl(Lwjgl3Application.GLDebugMessageSeverity, boolean)}
	 * to enable or
	...
	public static DisplayMode[] getDisplayModes() {
		...
		for (int i = 0; i < result.length; i++) {
			...
			result[i] = new Lwjgl3Graphics.Lwjgl3DisplayMode(GLFW.glfwGetPrimaryMonitor(), videoMode.width(),
					videoMode.height(),
					videoMode.refreshRate(), videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
		}
		return result;
	...
	static GridPoint2 calculateCenteredWindowPosition(Lwjgl3Monitor monitor, int newWidth, int newHeight) {
		...
		// If the new width is greater than the working area, we have to ignore stuff
		// like the taskbar for centering and use the
