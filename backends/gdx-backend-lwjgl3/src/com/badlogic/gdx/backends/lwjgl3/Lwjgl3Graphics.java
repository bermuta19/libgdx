...
public class Lwjgl3Graphics extends AbstractGraphics implements Disposable {
	 ...
	 * Should only be called if this feature is
	...
	public boolean setWindowedMode(int width, int height) {
		...
		if (!isFullscreen()) {
			...
			if (width != logicalWidth || height != logicalHeight) {
				...
				newPos = Lwjgl3ApplicationConfiguration.calculateCenteredWindowPosition((Lwjgl3Monitor) getMonitor(),
						width, height);
			}
		...
		} else { // if we were in fullscreen mode, we should consider restoring a previous
					// display mode
			...
			if (width != windowWidthBeforeFullscreen || height != windowHeightBeforeFullscreen) { // center the window
				...
				GridPoint2 newPos = Lwjgl3ApplicationConfiguration.calculateCenteredWindowPosition(
						(Lwjgl3Monitor) getMonitor(), width,
						height);
	...
	/**
	 * Sets the target framerate for the application, when using continuous
	 * rendering. Must be positive. The cpu sleeps as needed.
	 * Use 0 to never sleep. If there are multiple windows, the value for the first
	 * window created is used for all. Default is 0.
	 *
	 * @param fps fps
	 */
	@Override
	public void setForegroundFPS(int fps) {
		...
		if (getWindow().getConfig().activateBackgroundFPS == false) {
	...
	public void setBackgroundFPS(int fps) {
		getWindow().getConfig().backgroundFPS = fps;
		getWindow().getConfig().activateBackgroundFPS = true;
	}
