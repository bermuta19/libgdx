...
public class Lwjgl3Application implements Lwjgl3ApplicationBase {
		...
		while (running && windows.size > 0) {
			...
			for (Lwjgl3Window closedWindow : closedWindows) {
				if (windows.size == 1) {
					...
					// methods. The
					// application will be disposed when _all_ windows have been disposed, which is
					// the case,
	...
	/**
	 * Creates a new {@link Lwjgl3Window} using the provided listener and
	 * {@link Lwjgl3WindowConfiguration}.
	 *
	 * This function only just instantiates a {@link Lwjgl3Window} and returns
	 * immediately. The actual window creation is postponed
	 * with {@link Application#postRunnable(Runnable)} until after all existing
	 * windows are updated.
	 */
	...
	void createWindow(Lwjgl3Window window, Lwjgl3ApplicationConfiguration config, long sharedContext) {
		...
		if (currentWindow != null) {
			// the call above to createGlfwWindow switches the OpenGL context to the newly
			// created window,
			// ensure that the invariant "currentWindow is the window with the current
			// active OpenGL context" holds
	...
	private static void initiateGL(boolean useGLES20) {
		if (!useGLES20) {
			...
			try {
				...
				glVersion = new GLVersion(Application.ApplicationType.Desktop, versionString, vendorString,
						rendererString);
			} catch (Throwable e) {
	...
	public enum GLDebugMessageSeverity {
		...
		MEDIUM(GL43.GL_DEBUG_SEVERITY_MEDIUM, KHRDebug.GL_DEBUG_SEVERITY_MEDIUM,
	 ...
	 * Returns false if the severity level could not be
	 * set (e.g. the NOTIFICATION level is not supported by the ARB and AMD
	 * extensions).
	 *
	 * See
	...
	public static boolean setGLDebugMessageControl(GLDebugMessageSeverity severity, boolean enabled) {
		...
		if (caps.GL_ARB_debug_output && severity.arb != -1) {
			ARBDebugOutput.glDebugMessageControlARB(GL_DONT_CARE, GL_DONT_CARE, severity.arb, (IntBuffer) null,
					enabled);
			return true;
		}
