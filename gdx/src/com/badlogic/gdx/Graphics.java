 ...
 * Depending on the available hardware and the current
 * {@link Application} configuration, access to {@link GL20} and {@link GL30}
 * are provided here.
 * <p>
 * If supported by the backend, this interface lets you query the available
 * display modes (graphics resolution and color depth)
 * and change it.
 * <p>
 * This interface can be used to switch between continuous and non-continuous
 * rendering (see
 * {@link #setContinuousRendering(boolean)}), and to explicitly
 ...
 * {@link ShaderProgram} and {@link FrameBuffer},
 * {@link BitmapFont}, {@link Batch} and so on. All these classes are managed,
 * meaning they don't need to be reloaded on a context
 * loss. Explore the com.badlogic.gdx.graphics package for more classes that
 * might come in handy.
 * 
 * @author mzechner
 */
public interface Graphics {
	 ...
	 * of {@link GL30} via {@link #getGL30()} to
	 * access OpenGL ES 3.0 functionality. Note that this functionality will only be
	 * available if you instructed the
	...
	/**
	 * Returns whether OpenGL ES 3.1 is available. If it is you can get an instance
	 * of {@link GL31} via {@link #getGL31()} to
	 * access OpenGL ES 3.1 functionality. Note that this functionality will only be
	 * available if you instructed the
	...
	/**
	 * Returns whether OpenGL ES 3.2 is available. If it is you can get an instance
	 * of {@link GL32} via {@link #getGL32()} to
	 * access OpenGL ES 3.2 functionality. Note that this functionality will only be
	 * available if you instructed the
	...
	/**
	 * @return the inset from the left which avoids display cutouts in logical
	 *         pixels
	 */
	...
	/**
	 * @return the inset from the bottom which avoids display cutouts or floating
	 *         gesture bars, in logical pixels
	 */
	int getSafeInsetBottom();

	/**
	 * @return the inset from the right which avoids display cutouts in logical
	 *         pixels
	 */
	 ...
	 * that the id is incremented only when the
	 * application is in the running state right before calling the
	 * {@link ApplicationListener#render()} method. Also, the id of
	 * the first frame is 0; the id of subsequent frames is guaranteed to take
	 * increasing values for 2<sup>63</sup>-1 rendering
	 * cycles.
	 * 
	 ...
	 * android.util.DisplayMetrics#density, where one DIP is one pixel on an
	 * approximately 160 dpi screen. Thus on a 160dpi screen
	 * this density value will be 1; on a 120 dpi screen it would be .75; etc.
	 *
	 * If the density could not be determined, this returns a default value of 1.
	 *
	 * Depending on the underlying platform implementation this might be a
	 * relatively expensive operation. Therefore it should not
	 ...
	 * implementation. It may be necessary for the window to
	 * be recreated in order for the changes to take effect. Consult the
	 ...
	 * rendering. Might not work on all platforms. Is not
	...
	/**
	 * Sets the target framerate for the application when the application is not
	 * focused. Might not work on all platforms. Is not
	 * generally advised to be used on mobile platforms.
	 *
	 * @param fps the targeted fps; default differs by platform
	 */
	 ...
	 * {@link Application#postRunnable(Runnable)}. In the case of a
	 * multi-window app, all windows will request rendering if a runnable is posted
	 * to the application. To avoid this, post a
	 * runnable to the window instead.</li>
	 * </ul>
	 *
	 * Life-cycle events will also be reported as usual, see
	 * {@link ApplicationListener}. This method can be called from any
	 * thread.
	 *
	 ...
	 * format,
	 * width & height must be powers-of-two greater than zero (not necessarily
	 * equal) and of a certain minimum size (32x32 is a
	 * safe bet), and alpha transparency must be single-bit (i.e., 0x00 or 0xFF
	 * only). This function returns a Cursor object that
	 ...
	 * support cursor:url() and support the png format (the
	 * pixmap is converted to a data-url of type image/png) should also support
	 * custom cursors. Will set the mouse cursor image to
	 * the image represented by the {@link com.badlogic.gdx.graphics.Cursor}. It is
	 * recommended to call this function in the main
